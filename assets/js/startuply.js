...
;(function($){
	...
	$(".chk").change(function() {
		if(this.checked) {
			$($(this).data('id')).html('<span class="currency">$</span>' + $(this).data('website'));
		} else {
			$($(this).data('id')).html('<span class="currency">$</span>' + $(this).data('vy'));
		}
	});
...
Startuply = {
    ...
    formInit: function () {
        ...
        $(".close-btn, #confirmCode").on('click', function(e) {
            if($('.btnGetStarted')) {
                $('.btnGetStarted').val('GET STARTED');
                $('.btnGetStarted').attr('disabled', false);
            }
        });
        ...
        validateOptions = {
            ...
            submitHandler: function (form) {
				...
				if($('.btnRegisterNow')) {
					...
					$('.btnRegisterNow').attr('disabled', 'disabled');
				}
                
                if($('.btnGetStarted')) {
                    $('.btnGetStarted').val('Please wait..');
                    $('.btnGetStarted').attr('disabled', 'disabled');
                }
        ...
        doneHandler = function (msg, form) {
            ...
            if( msg === 'complete-finance' ) {
				
				window.location.href = "finance-broker-thankyou";
				
			} else if( msg === 'code-ok-finance' ) {
				
				window.location.href = "finance-broker-company-details";
				
			} else if( msg === 'code-failed' ) {
				if ( typeof toastr != 'undefined' ) toastr.error('The code you entered is incorrect, please check your mobile number is in international format.');
                else alert('The code you entered is incorrect, please check your mobile number is in international format.');
				
				$('[data-modal="4"]').modalBox('hide');
				$('#phone').val('+');
